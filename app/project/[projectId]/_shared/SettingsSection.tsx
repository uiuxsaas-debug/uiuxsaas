...
function SettingsSection({ projectDetail, screenDescription, takeScreenshot, screenConfig }: Props) {
    ...
    return (
        ...
        <div className={`relative h-[calc(100vh-80px)] border-r border-black/5 bg-white transition-all duration-300 ease-in-out flex flex-col z-[100] overflow-visible ${isOpen ? 'w-[320px]' : 'w-0'}`}>
            ...
            <Button
                ...
                className="absolute -right-6 top-6 h-6 w-6 rounded-full border border-black/10 shadow-md z-50 bg-white hover:bg-black/5 text-black p-0"
            >
            ...
            <div className={`flex-1 overflow-hidden flex flex-col ${!isOpen ? 'invisible' : 'visible'}`}>
                ...
                <div className="p-4 border-b border-black/5 bg-white relative z-10">
                    <div className='space-y-2'>
                        <label className='text-xs font-semibold uppercase text-black/50 tracking-wider'>Project Name</label>
                        <Input
                            placeholder='Project Name'
                            value={projectName}
                            className="bg-black/5 border-black/10 text-black placeholder:text-black/40 focus:border-[#FF5200]/50 transition-colors"
                            onChange={(event) => {
                ...
                <div className="flex-1 w-full overflow-y-auto p-4">
                    <div className='space-y-3 pb-4'>
                        <label className='text-xs font-semibold uppercase text-black/50 tracking-wider'>Color Theme</label>
                                ...
                                return (
                                    <div
                                        ...
                                        className={`
                                            ...
                                            hover:border-[#FF5200]/50 hover:shadow-sm hover:shadow-[#FF5200]/10
                                            ${isActive ? 'border-[#FF5200] bg-[#FF5200]/5 ring-1 ring-[#FF5200]/30' : 'border-black/5 bg-black/5'}
                                        `}
                                    >
                                        <div className="flex items-center justify-between mb-2">
                                            <h3 className={`text-xs font-medium ${isActive ? 'text-[#FF5200]' : 'text-black/70'}`}>{theme}</h3>
                                            {isActive && <div className="h-1.5 w-1.5 rounded-full bg-[#FF5200]" />}
                                        </div>
                ...
                <div className="border-t border-black/5 bg-white relative z-10">
                    <div className="p-4 pt-3 bg-black/5">
                        <h2 className='text-xs font-semibold uppercase text-black/50 tracking-wider mb-2'>Generate New Screen</h2>
                        <Textarea
                            placeholder='Describe the screen (e.g., "User Profile with charts")'
                            className="bg-white border-black/10 text-black placeholder:text-black/40 mb-2 min-h-[80px] resize-none text-sm focus:border-[#FF5200]/50"
                        ...
                        <Button
                            ...
                            className='w-full shadow-lg shadow-[#FF5200]/20 bg-[#FF5200] hover:bg-[#e04800] text-white font-semibold border-0'
